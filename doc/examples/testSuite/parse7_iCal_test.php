<?php // parse7_iCal_text.php, timezone input from file_get_contents

require_once '../iCalcreator.class.php';

$c = new vcalendar( array( 'unique_id' => 'test.se' ) );
$t = & $c->newComponent( 'timezone' );
$filetz = 'file folder'.DIRECTORY_SEPARATOR.'Europe_Madrid.ics';
// $contents = explode(PHP_EOL, @file_get_contents( $filetz )); // works
// $contents = @file_get_contents( $filetz ); // works
// $contents = file( $filetz ); // works, but with some extra eol
$contents = file( $filetz, FILE_IGNORE_NEW_LINES ); // works

if( FALSE === $t->parse( $contents ))
