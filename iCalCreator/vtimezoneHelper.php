...
/**
 * Return a formatted timestamp from the given timestamp.
 * 
 * Hybrid of date and mktime. Only those components specified in the
 * fomat are used in the creation of the new timestamp.
 * 
 * Deviating from the format string of the date function also a 
 * difference from the timestamp can be specified. The difference
 * is specified by following the component in the format string 
 * (that is after Y, m, d, H, i or s) by + or - followed by
 * an integer.
 * 
 * **Example:**
 * 
 *		datetime( 'HisYmd', $timestamp);
 *     // create an identical timestamp to $timestamp
 * 
 *		datetime( 'HisYmd', $timestamp) == datetime('YmdHis', $timestamp);
 *     // TRUE. Every permutation of the format string yields same timestamp
 * 
 *     datetime( 'Ymd', $timestamp);
 *     // create a timestamp where hours minutes and seconds are 0
 *  
 *     datetime( 'HisY+11md', $timestamp);
 *     // create a timestamp exactly 11 years in the future
 * 
 * @param string $format
 * @param int $timestamp
 * 
 * @return int
 */
function _datetime($format, $timestamp)
{	
	preg_match_all('/(?<p>[HisYmd])(?<op>[+-]?)(?<diff>\d*)/', $format, $matches, PREG_SET_ORDER);
	...
	// Every not defined format-Parameter is set to 0
	foreach( array('H','i','s','Y','m','d') as $p )
	{
		if( !isset($$p) )
		{
			$$p = 0;
