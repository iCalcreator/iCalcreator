...
define( 'ICALCREATOR_VERSION', 'iCalcreator 2.13dev' );
/*********************************************************************************/
/*********************************************************************************/
/**
 * vcalendar class
      ...
      case 'URL':
        ...
        foreach ( $this->components as $cix => $component) {
          if( !in_array( $component->objName, array('vevent', 'vtodo', 'vjournal', 'vfreebusy' )))
            continue;
          if( in_array( strtoupper( $propName ), array( 'ATTENDEE', 'CATEGORIES', 'CONTACT', 'RESOURCES' ))) {
          ...
          elseif( 'GEOLOCATION' == $propName ) {
            ...
            $content .= $sign.sprintf( '%8.6f', abs( $geo['longitude'] )).'/';   // sprintf && lpad && float && sign !"#¤%&/(
          }
 ...
 */
  function getComponent( $arg1=FALSE, $arg2=FALSE ) {
    ...
    elseif( is_array( $arg1 )) { // array( *[propertyName => propertyValue] )
      ...
      $otherProps = array( 'ATTENDEE', 'CATEGORIES', 'CONTACT', 'LOCATION', 'ORGANIZER', 'PRIORITY', 'RESOURCES', 'STATUS', 'SUMMARY', 'UID', 'URL' );
    }
      ...
      elseif( is_array( $arg1 )) { // array( *[propertyName => propertyValue] )
        ...
        foreach( $arg1 as $pName => $pValue ) {
          ...
          if( !in_array( $pName, $dateProps ) && !in_array( $pName, $otherProps ))
            continue;
          if(( 'ATTENDEE' == $pName ) || ( 'CATEGORIES' == $pName ) || ( 'CONTACT' == $pName ) || ( 'RESOURCES' == $pName )) { // multiple ocurrence may occur
            $propValues = array();
            $component->_getProperties( $pName, $propValues );
            $propValues = array_keys( $propValues );
            $hit = ( in_array( $pValue, $propValues )) ? TRUE : FALSE;
            continue;
          } // end   if(.. .// multiple ocurrence may occur
 ...
 */
  function selectComponents2( $selectOptions ) {
    $output = array();
    $allowedProperties = array( 'ATTENDEE', 'CATEGORIES', 'CONTACT', 'LOCATION', 'ORGANIZER', 'PRIORITY', 'RESOURCES', 'STATUS', 'SUMMARY', 'UID', 'URL' );
    foreach( $this->components as $cix => $component3 ) {
      ...
      foreach( $selectOptions as $propName => $pvalue ) {
        ...
        elseif(( 'ATTENDEE' == $propName ) || ( 'CATEGORIES' == $propName ) || ( 'CONTACT' == $propName ) || ( 'RESOURCES' == $propName )) { // multiple occurrence?
          $propValues = array();
...
/**
 * returns calendar property unique values for 'CATEGORIES', 'RESOURCES' or 'ATTENDEE' and for each, number of ocurrence
 *
 * @author Kjell-Inge Gustafsson, kigkonsult <ical@kigkonsult.se>
 * @since 2.12.8 - 2012-04-24
 * @param string $propName
 ...
 */
  function _getProperties( $propName, & $output ) {
    if( !in_array( strtoupper( $propName ), array( 'ATTENDEE', 'CATEGORIES', 'CONTACT', 'RESOURCES' )))
      return output;
    while(( FALSE !== ( $content = $this->getProperty( $propName ))) && !empty( $content )) {
      if( is_array( $content )) {
